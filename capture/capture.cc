...
public:
	...
	void setFormat( v4l2_buf_type type,
	...
	bool canCapture()  { return this->can( V4L2_CAP_VIDEO_CAPTURE ); }
	bool canStream()   { return this->can( V4L2_CAP_STREAMING );     }
	bool isCapturing() { return this->capturingIsActive;             }

	int getWidth()  { return FRAME_WIDTH;  }
	int getHeight() { return FRAME_HEIGHT; }
	int getColors() { return FRAME_COLORS; }
...
int main() {
	V4L2Device* dev0      = new V4L2Device( "/dev/video0" );
	unsigned char* frame0 = (unsigned char*)malloc( dev0->getFrameSize() );

	V4L2Device* dev1      = new V4L2Device( "/dev/video1" );
	unsigned char* frame1 = (unsigned char*)malloc( dev1->getFrameSize() );

	dev0->start();
	dev1->start();

	dev0->getFrame( frame0 );
	dev1->getFrame( frame1 );

	FILE* out0 = fopen("test0.rgb", "wb");
	FILE* out1 = fopen("test1.rgb", "wb");

	fwrite(frame0, 320 * 240 * 3, 1, out0);
	fwrite(frame1, 320 * 240 * 3, 1, out1);
